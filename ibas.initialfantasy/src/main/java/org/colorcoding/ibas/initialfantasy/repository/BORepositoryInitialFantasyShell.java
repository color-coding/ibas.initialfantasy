...
public class BORepositoryInitialFantasyShell extends BORepositoryInitialFantasy implements IBORepositoryShell {
	 ...
	 * 用户登录失败记录管理器
	 */
	private static final UserLoginFailTracker LOGIN_FAIL_TRACKER = UserLoginFailTracker.getInstance();
	...
	public OperationResult<User> userConnect(String user, String password) {
		try {
			...
			try (BORepositoryInitialFantasyShell boRepository = new BORepositoryInitialFantasyShell()) {
				...
				if (LOGIN_FAIL_TRACKER.isUserLocked(boUser.getCode())) {
					// [修复] 锁定提示不暴露用户名，与密码错误使用相同的提示风格
					throw new Exception(I18N.prop("msg_if_user_name_and_password_not_match"));
				}
				if (!boUser.checkPassword(password)) {
					// 登录失败，记录（统一用 boUser.getCode()）
					LOGIN_FAIL_TRACKER.recordLoginFail(boUser.getCode());
					throw new Exception(I18N.prop("msg_if_user_name_and_password_not_match"));
				}
				// 登录成功，清除失败记录
				LOGIN_FAIL_TRACKER.clearLoginFailLog(boUser.getCode());
				// 检查密码是否过期
	...
	public OperationResult<org.colorcoding.ibas.initialfantasy.bo.organization.User> fetchUser(ICriteria criteria,
		...
		for (org.colorcoding.ibas.initialfantasy.bo.organization.User item : operationResult.getResultObjects()) {
			item.setLocked(LOGIN_FAIL_TRACKER.getLockedStatus(item.getCode()));
		}
		return operationResult;
	}
	...
	public OperationResult<org.colorcoding.ibas.initialfantasy.bo.organization.User> saveUser(
		...
		// 仅超级用户可以解除锁定
		if (bo.getLocked() != emYesNo.YES && LOGIN_FAIL_TRACKER.hasLoginFailLog(bo.getCode())) {
			if (opUser != null) {
				LOGIN_FAIL_TRACKER.clearLoginFailLog(bo.getCode());
			}
		}
		// 更新时，恢复密码
		if (!bo.isDeleted()) {
