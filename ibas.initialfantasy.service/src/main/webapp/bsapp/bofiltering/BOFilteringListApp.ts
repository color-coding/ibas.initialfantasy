...
export class BOFilteringListApp extends ibas.BOListApplication<IBOFilteringListView, bo.BOFiltering> {
    ...
    /** 删除数据，参数：目标数据集合 */
    protected deleteData(data: bo.BOFiltering): void {
        // 检查目标数据
        if (ibas.objects.isNull(data)) {
            this.messages(ibas.emMessageType.WARNING, ibas.i18n.prop("sys_shell_ui_please_chooose_data",
        ...
        let beDeleteds:ibas.ArrayList<bo.BOFiltering> = new ibas.ArrayList<bo.BOFiltering>();
            ...
            onCompleted(action: ibas.emMessageAction): void {
                if (action === ibas.emMessageAction.YES) {
                    try {
                        ...
                        let saveMethod: Function = function(beSaved: bo.BOFiltering):void {
