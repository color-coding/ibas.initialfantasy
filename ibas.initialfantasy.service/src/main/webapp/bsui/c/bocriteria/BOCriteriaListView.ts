 ...
 * 视图-BOCriteria
 */
export class BOCriteriaListView extends ibas.BOListView implements IBOCriteriaListView {
    /** 返回查询的对象 */
    get queryTarget(): any {
                    ...
                    new sap.m.Button("", {
                        ...
                        press: function (): void {
                            that.fireViewEvents(that.viewDataEvent,
                                // 获取表格选中的对象
                                utils.getTableSelecteds<bo.BOCriteria>(that.table).firstOrDefault()
                            );
                        }
                    }),
                    new sap.m.Button("", {
                    ...
                    new sap.m.Button("", {
                        ...
                        press: function (): void {
                            that.fireViewEvents(that.deleteDataEvent,
                                // 获取表格选中的对象
                                utils.getTableSelecteds<bo.BOCriteria>(that.table)
                            );
                        }
                    }),
                ],
    ...
    showData(datas: bo.BOCriteria[]): void {
        ...
        if (!ibas.objects.isNull(model)) {
            // 已存在绑定数据，添加新的
            let hDatas: bo.BOCriteria[] = (<any>model).getData();
