...
import { IApplicationFunctionListView } from "../../../bsapp/applicationfunction/index";

/**
 * 视图-ApplicationFunction
 */
export class ApplicationFunctionListView extends ibas.BOListView implements IApplicationFunctionListView {
    /** 返回查询的对象 */
    get queryTarget(): any {
        return bo.ApplicationFunction;
    }
    /** 编辑数据，参数：目标数据 */
                    ...
                    new sap.m.Button("", {
                        ...
                        press: function (): void {
                            that.fireViewEvents(that.viewDataEvent,
                                // 获取表格选中的对象
                                utils.getTableSelecteds<bo.ApplicationFunction>(that.table).firstOrDefault()
                            );
                        }
                    }),
                    new sap.m.Button("", {
    ...
    showData(datas: bo.ApplicationFunction[]): void {
        ...
        if (!ibas.objects.isNull(model)) {
            // 已存在绑定数据，添加新的
            let hDatas: bo.ApplicationFunction[] = (<any>model).getData();
